...
var _resultsJs = require("./components/results.js");
...
class Search {
    ...
    async search(recipeName) {
        try {
            const res = await fetch(`${(0, _utilityJs.API_URL)}?search=${recipeName}&key=${(0, _utilityJs.API_KEY)}`);
...
},{"../utility.js":"2J5qt","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2J5qt":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Results {
    constructor(search){
        ...
        const renderResults = ()=>{
            ...
            if (this.page < this.totalPages) paginationHTML += `<button class="btn--inline pagination__btn--next">
                <span>Page ${this.page + 1}</span>
                <svg class="search__icon">
    ...
    constructor(results, bookmarks){
        ...
        <svg>
          <use href="src/img/icons.svg#icon-loader"></use>
            ...
            try {
                const res = await fetch(`${(0, _utilityJs.API_URL)}/${this.results.currentRecipeId}`);
                if (!res.ok) throw Error("Something went wrong.");
                const data = await res.json();
                this.recipe = data.data.recipe;
                for (const ingredient of this.recipe.ingredients)if (ingredient.quantity == null) ingredient.quantity = 1;
                renderRecipe();
            } catch (err) {
